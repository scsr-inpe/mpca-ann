...
FUNCTION integer8_to_string(integer_value, string_length)
    IMPLICIT NONE

    INTEGER (kind = 8), INTENT(IN) :: integer_value
    ...
    ! (used to make room for sign)
    CHARACTER (LEN = MAX(1 + FLOOR(LOG10(REAL(ABS(integer_value)))) +   &
                         INT((SIGN(1_8, -integer_value) + 1) / 2),           &
                         string_length, 1)) :: integer8_to_string
    ...
    IF (LEN_TRIM(string_buffer) < LEN(integer8_to_string)) THEN
        integer8_to_string = REPEAT(' ', LEN(integer8_to_string) - LEN_TRIM(string_buffer)) //   &
                            TRIM(string_buffer)
    ELSE
        integer8_to_string = TRIM(string_buffer)
    END IF
...
SUBROUTINE start_section2(text, style)
    IMPLICIT NONE
    ...
    IF (use_escape_codes) THEN
        IF (LEN(text) < section_width) THEN
            ! Fill section title with spaces for uniform section width
            CALL write_formatted(REPEAT('*', section_width), 'bright')
            WRITE(output_unit, *)
            CALL write_formatted( &
                REPEAT(' ', (section_width - (LEN(text)))/2), 'bright', &
                text, style, &
                REPEAT(' ', (section_width - (LEN(text)))/2), 'bright')
            WRITE(output_unit, *)    
            CALL write_formatted(REPEAT('*', section_width), 'bright')
